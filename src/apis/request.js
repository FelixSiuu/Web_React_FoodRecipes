...
export const randomRecipes = (number) => {
  return axios.get(`https://api.spoonacular.com/recipes/random?${key2}&number=${number}`)
}

// get search recipes
export const searchRecipes = (keyWord,number) => {
  return axios.get(`https://api.spoonacular.com/recipes/complexSearch?${key2}&query=${keyWord}&number=${number}`)
}